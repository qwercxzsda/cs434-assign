...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = {
    for {
      block_move <- b.legalNeighbors.toStream
    } yield (block_move._1, block_move._2 :: history)
  }
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
    ...
    def isNotExplored(block_move: (Block, List[Move])) = {
      !(explored contains block_move._1)
    }
  ...
  lazy val pathsToGoal: Stream[(Block, List[Move])] = {
    def reachedGoal(block_move: (Block, List[Move])) = {
      done(block_move._1)
    }

    pathsFromStart filter reachedGoal
  }
   ...
   * position.
   */
  lazy val solution: List[Move] = try {
    pathsToGoal.head._2
  } catch {
    case _: NoSuchElementException => List()
  }
}
