...
class HuffmanSuite extends FunSuite {
  ...
  test("until") {
    val leaflist = List(Leaf('e', 1), Leaf('t', 2), Leaf('x', 4))
    val combined = combine(combine(leaflist))
    assert(until(singleton, combine)(leaflist) === combined)
  }

  test("createCodeTree") {
    val leaflist = List(Leaf('e', 1), Leaf('t', 2), Leaf('x', 4))
    val combined = combine(combine(leaflist))
    assert(createCodeTree(string2Chars("textxxx")) === combined.head)
  }
  ...
  test("quickEncode2") {
    new TestTrees {
      val str = "sdxjfsddxjjjssffddxxxjjjjffds"
